...
with tab1:
    ...
    if hp_df is not None:
        ...
        fig = px.line(hp_df, x="alpha", y="eval_cumulative_reward", color="gamma", title="Cumulative Reward vs Learning Rate (Alpha) by Discount Factor (Gamma)")
...
with tab2:
    ...
    if seed_df is not None:
        ...
        fig = px.box(seed_df, y="eval_cumulative_reward", title="Distribution of Cumulative Reward across Training Seeds")
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.error("Data not found.")
...
with tab3:
    ...
    if cross_df is not None:
        ...
        fig = px.bar(cross_df, x="scenario", y="cumulative_reward", title="Cumulative Reward by Test Scenario")
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.error("Data not found.")
